...
class AbstractDB
{
    ...
    function findAll($dt = false)
    {
        ...
        if ($dt) {
            $this -> dataTable = $this -> mysql -> getRows($sql);
        } else {
            $this -> objects = $this -> mysql -> getObjects($sql);
        }
    ...
    function buildForm($ajax = false, $displayFrm = true)
    {
        ...
        foreach ($this -> form as $key => $formElement) {
            ...
            if ($formElement['label']) {
                $form -> append($frm, '<td width="30%"><label>'. $formElement['label'] .': <label></td>');
            } else {
                $form -> append($frm, '<td width="30%">&nbsp;</td>');
            }
            $form -> append($frm, '<td width="70%">'. $this -> formElement($key, $formElement) .'</td>');
        }
        if (!$ajax) {
            $form -> append($frm, $form -> formEndButton());
        }
        if ($displayFrm) {
            $form -> append($frm, $form -> formEnd());
        }
