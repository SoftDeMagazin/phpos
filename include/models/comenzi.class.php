...
class Comenzi extends AbstractDB
{
    ...
    function verificareStocuri()
    {
        ...
        if (isset($ComandaContinut -> objects)) {
            ...
            foreach ($ComandaContinut -> objects as $objContinut) {
                ...
                if ($prod -> verificareStoc($objContinut -> cantitate) == false) {
                    return false;
                }
            }
        }
    ...
    function verificareStocuriPlus()
    {
        ...
        if ($array) {
            foreach ($array as $ar) {
                ...
                if ($prod -> verificareStoc($ar['cant']) == false) {
                    return false;
                }
            }
        }
	...
	<tr class="'. $rowClass .'" onMouseOver="$(this).addClass(\'rowhover\')" onMouseOut="$(this).removeClass(\'rowhover\')" onClick="xajax_clickProdusComanda('. $objContinut -> comanda_continut_id .');">';
        if ($buttons) {
            $txt .= '<td><div align="center"><input name="chkContinutComanda[]" type="checkbox" value="'. $objContinut -> comanda_continut_id .'" checked></div></td>';
        }
    ...
    <tr class="rowhead">';
        if ($buttons) {
            $txt .= '<td width="2%"><div align="center"><strong></strong></div></td>';
        }
